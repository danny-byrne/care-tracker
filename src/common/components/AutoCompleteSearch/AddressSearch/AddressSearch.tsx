...
import { PointOfInterest, useSearchAddressLazyQuery } from 'src/graphQL/serverMocks/graphQLGeneratedCode';
...
const AddressSearch: React.FC<AddressSearchProps> = ({ formik, defaultTerm, placeholder }) => {
    ...
    const updateSearchTerm = async (searchTermDebounce: string) => {
        ...
        try {
            await setResultsFromApi(searchTermDebounce, location?.latitude, location?.longitude);
        } catch (error) {
            setDataResult(null);
        }
    };
    ...
    const { setFormik, setResultsFromApi, setDataResult, suggestions, searchLoading, viewCreators, clearFields } =
...
const useSearchAddressHooks = (formik, onSuggestionClicked) => {
    ...
    const [suggestions, setSuggestions] = useState<string[] | AddressSearchResult[]>([]);
    ...
    const mapSuggestionsToSearchResults = (dataResult) => {
        const suggestionsData = dataResult?.map((val: AddressResult) => {
            return new AddressSearchResult(val.id, val.address.freeTextDisplayAddress, val.address, val.poi);
        });
    ...
    const setResultsFromApi = async (searchTermDebounce: string, latitude?: number, longitude?: number) => {
        // Disable save after user has updated search term, the last action they take has to be selecting
        // the search term to populate
        clearFields();
    ...
    const setAddress = (prefix, addressValues, formik) => {
        ...
        if (addressLine1.length > 0) {
            formik.setFieldValue(`${prefix}address.addressLine1`, addressLine1.join(' '));
        }
        ...
        if (addressValues.extendedZipCode) {
            formik.setFieldValue(`${prefix}address.zipCode`, addressValues.extendedZipCode);
        } else if (addressValues.zipCode) {
            formik.setFieldValue(`${prefix}address.zipCode`, addressValues.zipCode);
        }

        formik.setFieldValue(`${prefix}address.displayAddress`, addressValues.freeTextDisplayAddress);
    };
    ...
    const setFormik = (suggestion: ISuggestionItem) => {
        ...
        if (formik.values.activity) {
            ...
            formik.setFieldValue('activity.address.freeTextAddress', addressValues.freeTextDisplayAddress);
        }
    ...
    const getSuggestionViews = (query: string, handleClickSearchBox: (suggestion) => void): JSX.Element[] => {
        ...
        suggestions.forEach((suggestion: string | AddressSearchResult, i: number) => {
            if (typeof suggestion === 'string') {
                views.push(
                    <DefaultListItem
    ...
    const AddressListItem: React.FC<IProviderListItemProps> = ({ suggestion, index, query, handleClickSearchBox }) => {
        return (
            <Link
    ...
    const DefaultListItem: React.FC<IDefaultListItemProps> = ({ suggestion, index, query, handleClickSearchBox }) => {
        return (
