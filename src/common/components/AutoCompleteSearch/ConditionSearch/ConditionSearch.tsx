...
const ConditionSearch: React.FC<ConditionSearchProps | MedicationConditionSearchProps> = ({
    formik,
    ...
    const onSuggestionClicked = (suggestion: any, handleClickSearchbox?: (suggestion) => void) => {
    ...
    const { setFormik, setResultsFromApi, setDataResult, suggestions, searchLoading, viewCreators, PLACEHOLDER_TEXT } =
        useSearchConditionHooks(formik, onSuggestionClicked, fromMedManagerView, isArray);
    ...
    return (
        ...
        <div>
            ...
            <Stack horizontal>
                ...
                />
                {searchLoading && (
                    <Spinner className={classNames['wc-ConditionSearch--spinner']} size={SpinnerSize.small} />
                )}
            </Stack>
        </div>
    );
...
const useSearchConditionHooks = (formik, onSuggestionClicked, fromMedManagerView, isArray) => {
    const classNames = getClassNames();
    ...
    class ConditionSearchResult implements ISuggestionItem {
        constructor(private conditionName: string, private icd10Code: string, private index: number) {}
    ...
    const [searchCondition, { loading: searchLoading }] = useConditionSearchLazyQuery({
        onError: () => {
    ...
    const setResultsFromApi = async (searchTermDebounce: string) => {
        const result = await searchCondition({ variables: { input: { searchText: searchTermDebounce } } });
    ...
    const setFormik = (suggestion: any) => {
        ...
        if (!isArray) {
            ...
            const conditionIndexInCurrentArray = formik.values.conditions?.find((condition) => {
    ...
    const ConditionListItem: React.FC<IConditionListItemProps> = ({
        ...
        return (
            ...
            >
                <Stack>
                    <HighlightTextView text={suggestion.conditionName} filter={query || ''} />
                </Stack>
            </Link>
        );
    };
