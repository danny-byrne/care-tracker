...
const ImmunizationSearch: React.FC<ImmunizationSearchProps> = ({
  formik,
  defaultTerm,
}) => {
...
const useSearchImmunizationHooks = (formik, onSuggestionClicked) => {
  ...
  const [suggestions, setSuggestions] = useState<
    string[] | ImmunizationSearchResult[]
  >([]);
  ...
  const [searchImmunizations, { loading: searchLoading }] =
    useSearchImmunizationLazyQuery({
      onError: () => {
  ...
  const setResultsFromApi = async (searchTermDebounce: string) => {
    ...
    const result = await searchImmunizations({
  ...
  const getSuggestionViews = (
    ...
    suggestions.forEach(
      (suggestion: string | ImmunizationSearchResult, i: number) => {
        if (typeof suggestion === "string") {
          views.push(
            <DefaultListItem
