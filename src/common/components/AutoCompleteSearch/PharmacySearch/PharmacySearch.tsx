...
const PharmacySearch: React.FC<PharmacySearchProps> = ({ formik, defaultTerm }) => {
    ...
    const onSuggestionClicked = (suggestion: ISuggestionItem, handleClickSearchbox?: (suggestion) => void) => {
        trackClick('pharmacy-suggestion');
...
const useSearchPharmacyHooks = (formik, onSuggestionClicked) => {
    ...
    const [suggestions, setSuggestions] = useState<string[] | PharmacySearchResult[]>([]);
    ...
    class PharmacySearchResult implements ISuggestionItem {
        constructor(private name: string, private id: string, private address: Address, private phoneNumber: string) {}
    ...
    const mapSuggestionsToSearchResults = (dataResult) => {
        ...
        const suggestionsData = filteredResults?.map((val: any) => {
            return new PharmacySearchResult(val.poi?.name, val.id, val.address, val.poi?.phoneNumber);
        });
    ...
    const [searchPharmacies, { loading: searchLoading }] = useSearchPharmacyLazyQuery({
        onError: () => {
    ...
    const setResultsFromApi = async (searchTermDebounce: string) => {
        ...
        const result = await searchPharmacies({ variables: { searchText: searchTermDebounce } });
    ...
    const setFormik = (suggestion: ISuggestionItem | string) => {
        ...
        if (suggestion === OTHER_STRING) {
            ...
            formik.setFieldValue('pharmacy.phoneNumber', formatPhoneNumber(typedSuggestion.getPhoneNumber()));
            ...
            formik.setFieldValue('pharmacy.location.freeTextAddress', address.freeTextDisplayAddress);
    ...
    const getSuggestionViews = (query: string, handleClickSearchBox: (suggestion) => void): JSX.Element[] => {
        ...
        suggestions.forEach((suggestion: string | PharmacySearchResult, i: number) => {
            if (typeof suggestion === 'string') {
                views.push(
                    <DefaultListItem
    ...
    const PharmacyListItem: React.FC<IPharmacyListItemProps> = ({ suggestion, index, query, handleClickSearchBox }) => {
        return (
            <Link
