...
import { capitalizeFirstLetterOfEachWord, capitalizeOnlyFirstLetter } from 'src/common/helpers/textFormatting';
...
const ProviderSearch: React.FC<ProviderSearchProps> = ({ formik, defaultTerm, isAdd }) => {
    ...
    const onSuggestionClicked = (suggestion: ISuggestionItem, handleClickSearchbox?: (suggestion) => void) => {
        trackClick('provider-suggestion');
...
const useSearchProviderHooks = (formik, onSuggestionClicked, isAdd) => {
    ...
    const [suggestions, setSuggestions] = useState<string[] | ProviderSearchResult[]>([]);
    ...
    const PLACEHOLDER_TEXT = isAdd ? 'Search a provider name or address' : 'Who prescribed this medication?';
    ...
    class ProviderSearchResult implements ISuggestionItem {
        ...
        getAddressText: () => string = () => {
            const addressTwoIfProvided = this.address.address2 ? ' ' + this.address.address2 + ',' : ',';
            const addressStringWithoutState = `${this.address.address1}${addressTwoIfProvided} ${this.address.city}`;
            const formattedAddress = capitalizeFirstLetterOfEachWord(addressStringWithoutState);
            const finalAddress = formattedAddress + `, ${this.address.state} ${this.address.zipCode}`;
            return finalAddress;
        };
    ...
    const mapSuggestionsToSearchResults = (dataResult) => {
        const suggestionsData = dataResult?.map((val: any) => {
            const address: Address = {
                ...
                telephoneNumber: val?.providerBusinessPracticeLocationAddressTelephoneNumber,
            };
    ...
    const [searchProviders, { loading: searchLoading }] = useSearchProviderLazyQuery({
        onError: () => {
    ...
    const setResultsFromApi = async (searchTermDebounce: string) => {
        ...
        const result = await searchProviders({ variables: { searchText: searchTermDebounce } });
    ...
    const getSuggestionViews = (query: string, handleClickSearchBox: (suggestion) => void): JSX.Element[] => {
        ...
        suggestions.forEach((suggestion: string | ProviderSearchResult, i: number) => {
            if (typeof suggestion === 'string') {
                views.push(
                    <DefaultListItem
    ...
    const ProviderListItem: React.FC<IProviderListItemProps> = ({ suggestion, index, query, handleClickSearchBox }) => {
        return (
            <Link
