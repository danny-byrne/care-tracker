...
const DayTime: React.FC<ISubHeaderProps> = (props) => {
    ...
    const getIcon = () => {
        ...
        } else if (timeInMinutes > timeStringToMinutes('9:00AM') && timeInMinutes <= timeStringToMinutes('5:00PM')) {
            return { icon: <DayTimeIcon />, color: colors.windcrest.daytime };
        } else if (timeInMinutes > timeStringToMinutes('5:00PM') && timeInMinutes < timeStringToMinutes('7:00PM')) {
    ...
    const timeContainer = mergeStyles(classNames['wc-Daytime--container'], { backgroundColor: color });

    return (
        <Stack className={classNames['wc-Daytime--wrapper']}>
            <Stack className={timeContainer} horizontal>
                <div className={classNames['wc-Daytime--iconContainer']} data-testid={testId}>
                    {icon}
                </div>
                <Text className={classNames['wc-Daytime--timeText']}>{asNeeded ? 'As Needed' : time}</Text>
            </Stack>
        </Stack>
    );
};
