...
import { DefaultButton, GroupedList, IGroup, IGroupHeaderProps, Spinner, Stack, Text, Image } from '@fluentui/react';
...
const Activities = () => {
    ...
    const { showAddPanelWithCustomParameter, showAddPanel, hideAddPanel } = useAddPanelControls();
    ...
    useEffect(() => {
        ...
        if (status === 'added') {
            removeSearchParam('status');
            setSuccessToast('Create another one', 'Activity Created', `Let's Go`, () => showAddPanel());
        }
    }, [status]);
        ...
        return (
            <>
                ...
                >
                    ...
                    <Text className={classNames['wc-Activities--header']}>{HEADER_LINE}</Text>
                    <Text className={classNames['wc-Activities--subHeader']}>{IDEAS_LINE}</Text>
                </Stack>
    ...
    const getActivityGroups = (activities: Activity[]): IGroup[] => {
        ...
        if (activities !== undefined) {
            groups.push({ key: 'All Activities', name: 'All Activities', startIndex: 0, count: activities.length });
        }
    ...
    const NewActivityButton = () => {
        return (
            <DefaultButton
                ...
                onClick={() => {
                    trackClick('create-activity-button');
                    showAddPanel();
                }}
            >
                Create New Activity
    ...
    const onRenderHeader = (props?: IGroupHeaderProps): JSX.Element | null => {
        if (props) {
            ...
            return (
                <Stack horizontal className={classNames['wc-Activities--allActivitiesHeader']} verticalAlign="center">
                    <Accordion
                        // eslint-disable-next-line react/prop-types
                    ...
                    <Stack horizontal tokens={{ childrenGap: 10 }} verticalAlign="center">
                        ...
                        {/* eslint-disable-next-line react/prop-types */}
                        {!isMobile && props?.group?.key === 'All Activities' && <NewActivityButton />}
                    </Stack>
                </Stack>
            );
        }
    ...
    return loading ? (
        ...
        <Stack className={classNames['wc-Activities--container']} tokens={{ childrenGap: 24 }}>
            {experiences?.length === 0 ? (
                ...
                <>
                    ...
                    <PrintableContentContainer ref={printActivitiesRef}>
                        <ActivitiesPrint experiences={experiences} recipientData={recipientData} />
