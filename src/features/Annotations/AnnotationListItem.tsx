...
import { Annotation, AnnotationType } from 'src/graphQL/serverMocks/graphQLGeneratedCode';
...
const AnnotationListItem: React.FC<IAnnotationListItemProps> = (props) => {
    ...
    return (
        <div
            ...
            data-is-focusable
            onClick={() => {
                trackClick('annotation-list-item');
                navigate(RouterConfig.Annotation(annotation.id));
            }}
        >
            <DocumentCard className={classNames['wc-AnnotationListItem--card']}>
                <Stack horizontal horizontalAlign="space-between" verticalAlign="center">
                    <Stack className={classNames['wc-AnnotationListItem--container']} tokens={{ childrenGap: 4 }}>
                        <Text className={classNames['wc-AnnotationListItem--type']} style={{ color: color }}>
                            {type}
                        </Text>
                        <Text className={classNames['wc-AnnotationListItem--date']}>{date.toLocaleString()}</Text>
                        <Text className={classNames['wc-AnnotationListItem--text']}>{text}</Text>
                    </Stack>
                    <IconButton
