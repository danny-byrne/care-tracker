...
import { getAdjustedTime, useGetTimezoneInfo } from 'src/common/hooks/useGetTimezoneInfo';
...
const AppointmentView: React.FC = () => {
    ...
    useEffect(() => {
        if (medicationData) {
            setMedications(medicationData.careRecipientMedicationPrescriptions?.prescriptions);
        }
    }, [medicationData]);

    useEffect(() => {
        if (data) {
            const selectedAppointment = data?.careRecipientAppointments?.appointments.filter(
    ...
    useEffect(() => {
        ...
        if (appointment) {
            ...
            dateText = `${monthsFullNames[startDate.getMonth()]} ${startDate.getDate()}`;
    ...
    const dateProps = [
        {
            content: (
                <Stack tokens={{ childrenGap: 8 }}>
                    <Text className={classNames['wc-AppointmentView--cardTitleText']}>{dateText}</Text>
                    <Text>{timeText}</Text>
                </Stack>
            ),
        },
    ...
    const medicationProps = [
        {
            content: (
                <Stack tokens={{ childrenGap: 8 }}>
                    <Text className={classNames['wc-AppointmentView--cardTitleText']}>Medications</Text>
                    {medications.map((medication) => {
                        return (
                            <Stack key={medication.id}>
                                <Text className={classNames['wc-AppointmentView--medTitleText']}>
                                    {medication.medication?.name} {medication.strengthValue}
                                </Text>
                                {medication.takenFor?.condition && (
                                    <Text className={classNames['wc-AppointmentView--medConditionText']}>
    ...
    const questionProps = [
        {
            content: (
                <Stack tokens={{ childrenGap: 8 }}>
                    <Text className={classNames['wc-AppointmentView--cardTitleText']}>Questions for Provider</Text>
                    {questionLoading ? (
                        <Spinner />
                    ) : (
                        questionData?.questionGenerate?.result?.questions.map((question) => (
                            <Text key={question} className={classNames['wc-AppointmentView--questionText']}>
                                {question}
                            </Text>
                        ))
                    )}
    ...
    // TODO: Implement symptoms when backend is finished
    // const { data: sypmtomData, loading: symptomLoading } = useGenerateSymptomsQuery();
    // const symptomProps = [
    //     {
    //         content: (
    //             <Stack tokens={{ childrenGap: 8 }}>
    //                 <Text className={classNames['wc-AppointmentView--cardTitleText']}>Symptoms</Text>
    //             </Stack>
    //         ),
    //     },
    // ];
    ...
    return (
        <>
            ...
            >
                ...
                <Stack horizontal>
                    <Back />
                    <ActionButtonRow inLine onDelete={toggleHideDeleteDialog} hideSeparator />
                </Stack>
                <Stack tokens={{ childrenGap: 24 }}>
                    <Stack tokens={{ childrenGap: 8 }}>
                        <Text className={classNames['wc-AppointmentView--titleText']}>{appointment?.description}</Text>
                        {appointment?.location?.freeTextAddress && <Text>{appointment.location.freeTextAddress}</Text>}
                    </Stack>

                    <ReusableCardList buttonPropsList={dateProps} />
                    <Stack tokens={{ childrenGap: 16 }}>
                        <Stack tokens={{ childrenGap: 4 }}>
                            <Text className={classNames['wc-AppointmentView--sectionHeaderText']}>Details</Text>
                            <Stack tokens={{ childrenGap: 16 }}>
                                {medications.length > 0 && <ReusableCardList buttonPropsList={medicationProps} />}
                ...
                >
                    <AppointmentEdit onDismiss={hideEditPanel} appointment={appointment} />
                </Panel>
            </SubHeaderLayout>
        </>
    );
