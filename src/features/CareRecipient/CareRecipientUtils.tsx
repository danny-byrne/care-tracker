...
const setInitValues = (dataProfile, photoChanged, timezone) => {
    ...
    const formattedPhoneNumber = dataProfile?.phone ? formatPhoneNumber(dataProfile.phone) : '';
    ...
    return {
        ...
        timezone: timezone,
        measurementSystemPreference: dataProfile?.measurementSystemPreference ?? MeasurementSystem.Imperial,
        feet: dataProfile?.height && imperial ? convertToFeet(dataProfile?.height) : '',
        inches: dataProfile?.height && imperial ? convertToInches(dataProfile?.height) : '',
        pounds:
            dataProfile?.weight && imperial ? Number((Number(dataProfile?.weight) * kilogramsToPounds).toFixed(2)) : '',
        meters: dataProfile?.height && !imperial ? Number(dataProfile?.height / 100).toFixed(0) : '',
        centimeters: dataProfile?.height && !imperial ? Number(dataProfile?.height % 100) : '',
        kilograms: dataProfile?.weight && !imperial ? Number(dataProfile?.weight) : '',
...
const onKeyDown = (ev: React.KeyboardEvent<HTMLInputElement | HTMLTextAreaElement>, pattern: RegExp) => {
...
const convertToFeet = (height) => {
    return ((Number(height) / centimetersToInches - Number(convertToInches(height))) / 12).toFixed(0);
};
