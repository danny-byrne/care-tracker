...
import { Dropdown, ResponsiveMode, DatePicker, Stack, PrimaryButton, DayOfWeek, Checkbox } from '@fluentui/react';
...
const MedicationsPrintPanel: React.FC<IMedicationPrintProps> = ({ onDismiss, meds }) => {
    ...
    const [medsDataForCalendarPrintView, setMedsDataForCalendarPrintView] = useState([]);
    ...
    useEffect(() => {
        ...
        const medicationsWithNoDosageInfo: Prescription[] = meds.filter((prescription) => {
            return !prescription.endDate && prescription.dosages.length === 0;
        });

        const medicationsScheduledAndNonScheduled: MedicationWithRRule[] | Prescription[] = [
        ...
        const medicationCalendarEvents: Array<ICalendarEvent> = createMedicationCalendarEventsBasedOnMedicationsData(
            medicationsFilteredByActiveAndWithDosage,
        ...
        const { medsPrintDisplay, titleColumns, pageLabel } = sortMedsToDisplayBasedOnFilters(
    ...
    useEffect(() => {
        ...
        if (filterValues.medsFilter === selectOptions.Today) {
            setFilterValues({ ...filterValues, printRangeStart: today.toUTCString(), printRangeEnd: today });
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [filterValues.medsFilter]);

    const medListViewSelectedAndMedsToPrint = medsToDisplayForListView.length && !filterValues.showAsCalendarSchedule;
    ...
    const printButtonText =
        medListViewSelectedAndMedsToPrint || calendarViewSelectedAndMedsToPrint ? 'Print' : 'No Medications to Print';
    ...
    const disablePrintButton =
        (filterValues.showAsCalendarSchedule && !medsDataForCalendarPrintView.length) ||
    ...
    return (
        <>
                ...
                <Stack verticalAlign="space-between" style={{ height: '95%' }}>
                    <Stack tokens={{ childrenGap: 10 }}>
                        ...
                        <Dropdown
                            ...
                            onChange={(_, item) => {
                                trackFieldChanged('medsFilter');
                                setFilterValues({ ...filterValues, medsFilter: String(item.key) });
                            }}
            ...
            <PrintableContentContainer ref={printMedicationsRef}>
                {showAsCalendarView ? (
                    <CalendarPrint recipientData={recipientData} weeksData={medsDataForCalendarPrintView} />
                ) : (
                    <MedicationsPrint
