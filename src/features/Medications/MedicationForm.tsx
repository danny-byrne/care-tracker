...
import { dropdownErrorStyles, datepickerErrorStyles } from 'src/features/Medications/constants';
...
const MedicationForm: React.FC<IMedicationFormProps> = (props) => {
    ...
    const [searchCurrentProviders, { loading: searchProvidersLoading }] = useGetProvidersLazyQuery({
        onError: () => {
            setErrorToast(ERROR_MESSAGES.GET_PROVIDERS);
        },
    });

    const [searchCurrentPharmacies, { loading: searchPharmaciesLoading }] = useGetPharmaciesLazyQuery({
        onError: () => {
    ...
    useEffect(() => {
        const getProviders = async () => {
            ...
            const avaiableProviders = results.data.careRecipientProviders.providers.filter(
    ...
    useEffect(() => {
        const getConditions = async () => {
            ...
            const activeConditions = results.data.careRecipientConditions.conditions.filter(
                (condition) =>
    ...
    const activePharmacies = currentPharmacies.filter((pharmacy) => pharmacy.recordStatus == RecordStatus.Active);

    const getPharmacyDropdown = () => {
        const pharmacyKeys = activePharmacies.map((pharmacy) => {
            return {
                key: pharmacy.id,
                text: `${pharmacy.name}${pharmacy.location.singleLineAddress ? ', ' : ''}${
    ...
    const getConditionDropdown = () => {
        ...
        const output = [existingConditionsHeader, ...conditionDropdownOptions, addNew];
    ...
    const providersHaveBeenLoadedAndAreEmpty =
        providerDataLoaded && !showProviderDropdown && formik.values.provider.id !== addNewText;

    const pharmaciesHaveBeenLoadedAndAreEmpty =
        pharmacyDataLoaded && !showPharmacyDropdown && formik.values.pharmacy.id !== addNewText;
    ...
    const requireProvider =
        !formik.values.overTheCounter && showProviderDropdown && formik.values.provider.id === addNewText;

    const refillHasError = formik.touched.refillDate && (formik.errors?.refillDate as string);
        ...
        <>
                ...
                <Dropdown
                    ...
                    onChange={(_, item) => {
                        if (item.key === addNewText) {
                            ...
                            formik.setFieldValue('condition.conditionId', getConditionId(item));
            ...
            {!formik.values.overTheCounter && (
                <>
                    ...
                    {showProviderSearch && (
                        <ProviderForm formik={formik} providerRequired={requireProvider} errors={formik.errors} />
                    )}
                    <Checkbox
                    ...
                    {formik.values.refillChecked && (
                        <Stack>
                            <DatePicker
                                ...
                                onAfterMenuDismiss={() => formik.setTouched({ ...formik.touched, refillDate: true })}
                            />
