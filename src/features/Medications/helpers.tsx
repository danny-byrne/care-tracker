...
const getDailyScheduleText = (recurrenceFrequency, frequencyInterval) => {
    ...
    switch (true) {
        case recurrenceFrequency === 1:
            frequencyIntervalText = capitalizeOnlyFirstLetter(frequencyInterval.toLowerCase());
            break;
        case recurrenceFrequency > 1:
...
export const getFrequencyHeaderAndContent = (prescription: MedicationWithRRule, careRecipientTimezone?: string) => {
    let frequencyText = '',
    ...
    const medStartDate = new Date(rRuleParams?.scheduleStartDate).toLocaleDateString();
    ...
    if (prescription && prescription.rRuleParams) {
        ...
        ({ timeFormatted, recurrenceFrequencyCount, daysOfWeekParsed } = rRuleParams);
    }
    ...
    const { frequencyIntervalText } = getDailyScheduleText(recurrenceFrequencyCount, frequencyBaseValue);

    const daysOfWeekText = daysOfWeekParsed?.length ? `On ${daysOfWeekParsed.join(', ')}` : null;

    const frequencyTextBasedOffFrequencyAndWeekDays = daysOfWeekText ?? `${frequencyIntervalText}`;

    frequencyText = `${frequencyTextBasedOffFrequencyAndWeekDays} starting ${medStartDate}`;
    dosageInformation = `Take ${doseValue} ${doseUnit}`;
    frequencyContent = (
        <Stack horizontal tokens={{ childrenGap: 10 }} verticalAlign="center" key={medTimeDisplay}>
...
export const getCustomScheduleErrorsAndTouchedValues = (touched, errors, currentFormikPathIndex) => {
    ...
    const setIntervalDoseHasError = setIntervalDoseHasBeenTouched && setIntervalsDoseError;
    ...
    const intervalTimeError = errors?.medicationCourse?.[currentFormikPathIndex]?.scheduleInterval;
    const setIntervalIntervalHasBeenTouched = currentMedicationCourseTouched?.length
        ? currentMedicationCourseTouched[currentFormikPathIndex]?.scheduleInterval
        : null;

    const intervalTimeHasError = setIntervalIntervalHasBeenTouched && intervalTimeError;
    ...
    const startTimeHasError = errors?.medicationCourse?.[currentFormikPathIndex]?.scheduleIntervalStartTime;
    const startTimeHasBeenTouched = currentMedicationCourseTouched?.length
        ? currentMedicationCourseTouched[currentFormikPathIndex]?.scheduleIntervalStartTime
        : null;

    const intervalStartTimeHasError = startTimeHasBeenTouched && startTimeHasError;
    ...
    const endTimeHasError = errors?.medicationCourse?.[currentFormikPathIndex]?.scheduleIntervalEndTime;

    const endTimeHasBeenTouched = currentMedicationCourseTouched?.length
        ? currentMedicationCourseTouched[currentFormikPathIndex]?.scheduleIntervalEndTime
        : null;
